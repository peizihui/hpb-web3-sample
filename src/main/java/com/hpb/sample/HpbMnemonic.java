...
import io.hpb.web3.crypto.Wallet;
import io.hpb.web3.crypto.WalletFile;
 ...
 */
public class HpbMnemonic {
	/**
	 * 通用的以太坊基于bip44协议的助记词路径 （imtoken jaxx Metamask myhpberwallet）
	 */
...
//		importMnemonic(ETH_TYPE, list, "11111111");
	...
	public static HpbHDWallet generateMnemonic(String path, String password) {
		...
		return creatHpbWallet(ds, pathArray, password);
	}

	private static HpbHDWallet importMnemonic(String path, List<String> list, String password) {
		if (!path.startsWith("m") && !path.startsWith("M")) {
			//参数非法
			return null;
		}
	...
	private static HpbHDWallet creatHpbWallet(DeterministicSeed ds, String[] pathArray, String password) {
		//根私钥
		...
		System.out.println("hpb privateKey " + keyPair.getPrivateKey().toString(16));
		System.out.println("hpb publicKey " + keyPair.getPublicKey().toString(16));

	HpbHDWallet hpbHDWallet = null;
		try {
			WalletFile walletFile = Wallet.createLight(password, keyPair);
			System.out.println("hpb address " + "0x" + walletFile.getAddress());
			ObjectMapper objectMapper = ObjectMapperFactory.getObjectMapper();
			//存
			String jsonStr = objectMapper.writeValueAsString(walletFile);
			System.out.println("hpb keystore " + jsonStr);

			hpbHDWallet = new HpbHDWallet(keyPair.getPrivateKey().toString(16),
	...
	public static class HpbHDWallet {
		String privateKey;
		String publicKey;
		...
		public HpbHDWallet(String privateKey, String publicKey, List<String> mnemonic, String mnemonicPath, String address, String keystore) {
