...
//	private static String toAddress = "0x05f50cd5a97d9b3fec35df3d0c6c8234e6793bdf";
    ...
    private static String fromAddress = "0xdbc3f6a340eb8d065c6ccb06437a34d984d61134";
    private static String toAddress = " 0xbfdc9d8da67abd04a0fdbb9ebd593687801b3885";
	...
	private static Transaction makeTransaction(String fromAddress, String toAddress,
		...
		transaction = Transaction.createHpberTransaction(fromAddress, nonce, gasPrice, gasLimit, toAddress, value);
		return transaction;
	}
	...
	private static BigInteger getTransactionGasLimit(Transaction transaction) {
		...
		try {
		HpbEstimateGas hpbEstimateGas = web3.hpbEstimateGas(transaction).send();
			gasLimit = hpbEstimateGas.getAmountUsed();
		} catch (IOException e) {
			e.printStackTrace();
		}
		return gasLimit;
	...
	private static BigInteger getTransactionNonce(String address) {
		...
		try {
		HpbGetTransactionCount hpbGetTransactionCount = web3.hpbGetTransactionCount(address, DefaultBlockParameterName.PENDING).send();
	...
	private static String sendTransaction() {
		...
		try {
			...
			if (personalUnlockAccount.accountUnlocked()) {
				BigInteger value = Convert.toWei(amount, Convert.Unit.HPB).toBigInteger();
			...
			HpbSendTransaction hpbSendTransaction = web3.hpbSendTransaction(transaction).send();
	...
	//使用 web3.hpbSendRawTransaction() 发送交易 需要用私钥自签名交易 详见ColdWallet.java
}
